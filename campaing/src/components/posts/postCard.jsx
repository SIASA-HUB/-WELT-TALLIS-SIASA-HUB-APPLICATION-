...
// Base URL
const API_BASE_URL =
  "https://shaved-finest-instruction-franklin.trycloudflare.com";
...
const PostCard = ({ searchQuery = "" }) => {
  ...
  const filterPosts = (query, posts) => {
    ...
    function basicSearch(term, post) {
      ...
      if (post.content) {
        ...
        if (
          contentWords.some((word) => calculateSimilarity(word, term) > 0.6)
        ) {
          return true;
        }
      }
        ...
        {parts.map((part, index) => {
          if (part.startsWith("#")) {
            return (
              <span
                key={index}
                style={{ color: KENYA_COLORS.primary, fontWeight: "bold" }}
              >
                {part}
              </span>
            );
    ...
    try {
      ...
      const transformedVideos = videosArray.map((video, index) => ({
        id:
          ...
          video._id ||
          video.public_id ||
          `video_${postId}_${index}_${Date.now()}`,
  ...
  const transformPostData = (rawPost) => {
    ...
    const timestamp =
      rawPost.created_at || rawPost.timestamp || new Date().toISOString();
    ...
    return {
      ...
      post_id: postId,
      author:
        rawPost.author || `User ${rawPost.user_id?.slice(-4) || "Unknown"}`,
      party: extractParty(),
      title: rawPost.title || "",
  ...
  const fetchPosts = async (pageNum = 1, isLoadMore = false) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/posts/get?page=${pageNum}&limit=10`,
      );
      const data = await response.json();
  ...
  return (
    <>
      ...
      {displayPosts.map((post, index) => {
        ...
        const isLastPost =
          index === displayPosts.length - 1 && !searchQuery.trim();
            ...
            {showVideoBackups[post.post_id] && (
              ...
              >
                <VideoBackups
                  ...
                  videos={backupVideos}
                  onAddVideo={(newVideo) =>
                    handleAddBackupVideo(post.post_id, newVideo)
                  }
                  onClose={() =>
                    setShowVideoBackups((prev) => ({
                      ...prev,
            ...
            {showComments[post.post_id] && (
              <CommentsSection
                ...
                onAddComment={(newComment) =>
                  handleAddComment(post.post_id, newComment)
                }
                onClose={() =>
                  setShowComments((prev) => ({
                    ...prev,
