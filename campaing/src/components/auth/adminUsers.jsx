...
const UsersAdmin = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true);
      const response = await API_BASE_URL.get("/users/analytics");
  ...
  const exportData = () => {
    ...
    const csvData = [
      ...
      ...(data.generation || []).map(gen => [gen.generation, gen.count, `${gen.percentage}%`]),
      "",
      ["Political Parties", "Count", "Percentage"],
      ...(data.politicalParties || []).map(party => [party.political_party, party.count, `${party.percentage}%`]),
    ];
  ...
  if (!data) {
    return (
      <LoadingContainer>
        ...
        <p className="text-danger fw-semibold">Failed to load analytics data</p>
        <button className="btn btn-danger rounded-pill px-4 py-2" onClick={fetchAnalytics}>
  ...
  const maleCount = data.gender?.male || 0;
  const femaleCount = data.gender?.female || 0;
  const otherCount = data.gender?.other || 0;
  const votersWithCard = data.voterCard?.yes || 0;
  const willVoteCount = data.willVote?.yes || 0;

  // Calculate monthly trend
  const monthlyTrend = data.monthlyTrend || [];
  const maxNewUsers = Math.max(...monthlyTrend.map(m => m.new_users), 1);
  const totalNewUsers = monthlyTrend.reduce((sum, m) => sum + m.new_users, 0);
      ...
      <div className="container-fluid">
        ...
        <StatsGrid>
          <StatCard>
            ...
            <StatTrend $positive>
              <ArrowUp size="14" /> +{data.newUsersThisMonth} this month
            </StatTrend>
          </StatCard>
          ...
          <StatCard>
            ...
            <StatValue>{data.activeUsers || 0}</StatValue>
        ...
        {/* Monthly Growth Chart - Sleek Version */}
        <Section>
          <SectionTitle>
          ...
          <MonthlyChartContainer>
            ...
            <ChartWrapper>
              <BarsContainer>
                {monthlyTrend.map((month, index) => {
                  const barHeight = (month.new_users / maxNewUsers) * 200;
                  const isHighest = month.new_users === maxNewUsers;
                  const isLowest = month.new_users === Math.min(...monthlyTrend.map(m => m.new_users));
                  
                  return (
                    <MonthlyBar key={month.month}>
                      <MonthlyBarFill 
                      ...
                      />
                      <MonthlyValue>+{month.new_users}</MonthlyValue>
                      <MonthlyLabel>
            ...
            <div className="text-center mt-4 pt-2">
              <small className="text-white-50">
                ...
                Peak month: {Math.max(...monthlyTrend.map(m => m.new_users))} new users
              </small>
            </div>
          </MonthlyChartContainer>
        </Section>
          ...
          <ChartCard>
            ...
            <BarContainer>
              {(data.generation || []).map((gen) => (
                <BarItem key={gen.generation}>
                  <BarLabel>
                    <span>📅 {gen.generation}</span>
                    <span className="fw-bold text-cyan">{gen.count} ({gen.percentage}%)</span>
                  </BarLabel>
                  <Bar>
                    <BarFill $width={gen.percentage} $color="linear-gradient(90deg, #06b6d4, #0891b2)" />
                  </Bar>
                </BarItem>
              ))}
            </BarContainer>
            ...
            <BarContainer>
              <BarItem>
                <BarLabel>
                  ...
                  <span className="fw-bold text-success">{data.voterCard?.yes || 0} ({getPercentage(data.voterCard?.yes || 0, totalUsers)}%)</span>
                </BarLabel>
                <Bar>
                  <BarFill $width={getPercentage(data.voterCard?.yes || 0, totalUsers)} $color="linear-gradient(90deg, #22c55e, #16a34a)" />
                </Bar>
              </BarItem>
              <BarItem>
                <BarLabel>
                  <span>❌ No Voter Card</span>
                  <span className="fw-bold text-danger">{data.voterCard?.no || 0} ({getPercentage(data.voterCard?.no || 0, totalUsers)}%)</span>
                </BarLabel>
                <Bar>
                  <BarFill $width={getPercentage(data.voterCard?.no || 0, totalUsers)} $color="linear-gradient(90deg, #ef4444, #dc2626)" />
                </Bar>
              </BarItem>
            </BarContainer>
          </ChartCard>
            ...
            <BarContainer>
              <BarItem>
                <BarLabel>
                  ...
                  <span className="fw-bold text-success">{data.willVote?.yes || 0} ({getPercentage(data.willVote?.yes || 0, totalUsers)}%)</span>
                </BarLabel>
                <Bar>
                  <BarFill $width={getPercentage(data.willVote?.yes || 0, totalUsers)} $color="linear-gradient(90deg, #22c55e, #16a34a)" />
                </Bar>
              </BarItem>
              <BarItem>
                <BarLabel>
                  <span>❌ Will Not Vote</span>
                  <span className="fw-bold text-danger">{data.willVote?.no || 0} ({getPercentage(data.willVote?.no || 0, totalUsers)}%)</span>
                </BarLabel>
                <Bar>
                  <BarFill $width={getPercentage(data.willVote?.no || 0, totalUsers)} $color="linear-gradient(90deg, #ef4444, #dc2626)" />
                </Bar>
              </BarItem>
              <BarItem>
                <BarLabel>
                  <span>🤔 Not Sure</span>
                  <span className="fw-bold text-orange">{data.willVote?.notSure || 0} ({getPercentage(data.willVote?.notSure || 0, totalUsers)}%)</span>
                </BarLabel>
                <Bar>
                  <BarFill $width={getPercentage(data.willVote?.notSure || 0, totalUsers)} $color="linear-gradient(90deg, #f59e0b, #ea580c)" />
                </Bar>
              </BarItem>
            </BarContainer>
          </ChartCard>
        ...
        <Grid2Col>
          ...
          <ChartCard>
            ...
            <BarContainer>
              {Object.entries(data.employment || {}).map(([status, count]) => (
                <BarItem key={status}>
                  <BarLabel>
                    <span>💼 {status}</span>
          ...
          <ChartCard>
            ...
            <BarContainer>
              {(data.politicalParties || []).slice(0, 5).map((party) => (
                <BarItem key={party.political_party}>
                  <BarLabel>
                    <span>🏛️ {party.political_party}</span>
                    <span className="fw-bold text-red">{party.count} ({party.percentage}%)</span>
                  </BarLabel>
                  <Bar>
                    <BarFill $width={party.percentage} $color="linear-gradient(90deg, #ef4444, #dc2626)" />
                  </Bar>
                </BarItem>
              ))}
            </BarContainer>
        ...
        <Grid2Col>
          ...
          <ChartCard>
            ...
            <BarContainer>
              {(data.verification || []).map((item) => (
                <BarItem key={item.status}>
                  <BarLabel>
                    <span>{item.status === "Not Verified" ? "⚠️ Not Verified" : "✅ Verified"}</span>
                    <span className="fw-bold text-yellow">{item.count} ({item.percentage}%)</span>
                  </BarLabel>
                  <Bar>
                    <BarFill $width={item.percentage} $color="linear-gradient(90deg, #f59e0b, #d97706)" />
                  </Bar>
                </BarItem>
              ))}
            </BarContainer>
          ...
          <ChartCard>
            ...
            <BarContainer>
              {Object.entries(data.politicalLeanings || {}).map(([leaning, count]) => (
                <BarItem key={leaning}>
                  <BarLabel>
                    <span>⚖️ {leaning}</span>
        ...
        <div className="text-center mt-5 pt-4">
          <small className="text-muted">
            Data is updated in real-time | Last sync: {new Date(data.lastUpdated).toLocaleString()}
          </small>
        </div>
      </div>
    </DashboardContainer>
