// BoostedStoriesRow.js - Fixed (circle view videos working)
...
// Helper to check if message is auto-generated
const isAutoGeneratedMessage = (message) => {
  if (!message) return true;
  const autoMessages = ["📷 Photo", "🎥 Video", "💬 Support message", "Support message", "Photo", "Video"];
  return autoMessages.includes(message.trim());
};

// ============================================
// VIDEO THUMBNAIL COMPONENT FOR CIRCLE VIEW
// ============================================
const VideoThumbnail = ({ videoUrl, posterUrl }) => {
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      {!isPlaying ? (
        <>
          {posterUrl ? (
            <img src={posterUrl} alt="Video" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ) : (
            <div style={{ width: '100%', height: '100%', background: '#1e3c72', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px' }}>
              🎬
            </div>
          )}
...
const BoostedStoriesRow = ({
  ...
  // Get media URL - cleans and returns direct /uploads/ URL
  ...
  const getDisplayMessage = (story) => {
    ...
    // NO text for videos or images
    if (media_type === "video" || media_type === "image") {
      return null;
    }

    // Don't show auto-generated messages
    if (isAutoGeneratedMessage(message)) {
      return null;
    }
      ...
      <Section>
        ...
        <StoriesContainer>
          {stories.map((story, index) => {
            ...
            // Circle view - with playable video thumbnail
            return (
              <StoryItem
                key={story.id || index}
                onClick={() => handleStoryClick(story, index)}
                  ...
                  <StoryAvatar>
                    {mediaUrl && story.media_type !== "text" ? (
                      isVideoStory ? (
                        <VideoThumbnail
                          videoUrl={mediaUrl}
                          posterUrl={story.thumbnail_url ? buildImageUrl(story.thumbnail_url) : null}
                        />
                      ) : (
                        <img
                          src={mediaUrl}
