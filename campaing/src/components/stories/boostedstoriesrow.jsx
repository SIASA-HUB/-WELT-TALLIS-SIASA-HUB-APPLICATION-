// BoostedStoriesRow.js - Fixed (removes text below videos)
...
const BoostedStoriesRow = ({
  ...
  // Get media URL - uses buildImageUrl which now cleans corrupted URLs
  ...
  // Get display message - hides auto-generated text for videos/images
  const getDisplayMessage = (story) => {
    const { message, media_type } = story;

    // Don't show any message for videos or images
    if (media_type === "video" || media_type === "image") {
      return null;
    }

    // Don't show auto-generated messages
    if (isAutoGeneratedMessage(message, media_type)) {
      return null;
    }
            ...
            if (useCardView) {
              return (
                ...
                >
                  ...
                  <CardContent>
                    {/* Only show message if it's text story OR has real message */}
                    {displayMessage && (
                      <CardMessage>
                        {displayMessage}
                      </CardMessage>
