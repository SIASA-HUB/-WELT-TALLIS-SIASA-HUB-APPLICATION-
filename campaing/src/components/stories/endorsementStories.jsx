...
const EndorsementStories = ({ leaderId, currentUser, onBoostSuccess }) => {
      ...
      return (
        <>
          <video
            ...
            preload="metadata"
            onLoadedMetadata={(e) => {
              e.target.currentTime = 1; // Show first second as thumbnail
            }}
            onError={(e) => {
              console.warn(`Failed to load video: ${imageSrc}`);
              e.target.style.display = "none";
              if (e.target.parentElement) {
                const fallback = document.createElement("div");
                fallback.style.cssText =
  ...
  const validEndorsements = endorsements.filter((e) => {
    // STRICT 24 HOUR FILTER (WhatsApp Style)
    ...
    // If story is older than 24 hours, hide it
    if (now - storyDate > twentyFourHoursInMs) return false;

    // Also filter out generic/empty messages if they don't have media
