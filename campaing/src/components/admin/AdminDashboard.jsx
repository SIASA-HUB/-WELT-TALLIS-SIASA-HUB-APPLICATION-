...
// API Configuration
import API_BASE_URL from "../leaders/apiConfig";
...
const AdminDashboard = () => {
  ...
  const [stats, setStats] = useState({ aspirants: 0, pending: 0, wallets: 0, totalPoints: 0 });
  ...
  const fetchData = async () => {
    try {
      ...
      if (activeTab === "aspirants") {
        const res = await api.get("/leaders/popular"); // Using public endpoint for now or create admin one
        const allRes = await api.get("/leaders"); // Grouped feed
        ...
        const res = await axios.get(`${API_BASE_URL.replace('8009', '8005')}/wallet/admin/transactions`);
        // Note: In real app, all these would go through gateway 8009
        const walletRes = await axios.get(`${API_BASE_URL}/wallet/admin/transactions`); 
        setData(walletRes.data.data || []);
      }
    } catch (err) {
      ...
      <StatsGrid>
        <StatCard $bg="#e7e7ff" $color="#4318ff">
          ...
          <div className="details">
            <div className="label">Total Aspirants</div>
        ...
        <StatCard $bg="#e6faf5" $color="#05cd99">
          ...
          <div className="details">
            <div className="label">System Points</div>
            <div className="value">Social Hub</div>
          </div>
        </StatCard>
      ...
      <ContentCard>
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h2 style={{ fontSize: '20px', fontWeight: 800, margin: 0 }}>
            {activeTab === "aspirants" ? "Registered Political Aspirants" : "Global Wallet Transactions"}
          </h2>
          <SearchBar>
            <Search size={18} />
            <input 
              type="text" 
              placeholder={activeTab === "aspirants" ? "Search by name, county..." : "Search transaction, user..."}
              value={searchTerm}
                ...
                {activeTab === "aspirants" ? (
                  ...
                  filteredData.map((tx) => (
                    <tr key={tx.transaction_id}>
                      <td><code style={{ fontSize: 11 }}>{tx.transaction_id.slice(0, 15)}...</code></td>
                      <td>{tx.user_id}</td>
